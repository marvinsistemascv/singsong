...
(function ($) {
  'use strict'
    ...
    setTimeout(function () {
        ...
        if (window.___browserSync___ === undefined && Number(localStorage.getItem('Marvin Psicologos')) < Date.now()) {
            localStorage.setItem('Marvin Psicologos', (Date.now()) + (1 * 60 * 12000))

            Swal.fire({
                title: "um novo ano começou !, e com ele novas funções estão disponíveis no seu sistema, entre eles agora você conta com um chat entre os usuários do sistema" +
                    " bem como a equipe multifuncional, também será possível a impressão das tratativas/devolutivas, visualização de normativas e a possibilidade de enviar dúvidas ou sugestões"+
                    ", não se preocupe essa tela de aviso não será permanente !",
                width: 600,
                padding: "3em",
                color: "#716add",
                background: "#FFF0F5",
